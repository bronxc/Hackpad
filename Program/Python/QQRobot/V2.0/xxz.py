...
class SmallZ(HttpClient):
	...
	def getMessageGroup(self, key):
		...
		#split -ques
		if key.startswith("-ques ") or key.startswith("-api "):
			strbuffer = key.split(" ")
			key = strbuffer[0]
			self.parameter = strbuffer[1]
		
		#split -fight, and There is exists Eggs
		if key.startswith("-fight@"):
			strbuffer = key.split("@")
			key = strbuffer[0]
			self.parameter = strbuffer[1][0:20]
		else:
			pass
		
		#parameter Handle
		if self.parameter == "":
			self.parameter = "blank"
